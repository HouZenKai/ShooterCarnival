...
const PARALLAX_REPEAT_WIDTH : int = 1152 # Matches viewport width for seamless tiling
const PARALLAX_REPEAT_HEIGHT : int = 676 # Slightly less than 678 to create overlap and prevent seams
...
const BACKGROUND_SCROLL_SCALE : float = 0.3 # Far background moves slowest
const STARS_SCROLL_SCALE : float = 0.5 # Stars layer moves at medium speed
...
const BACKGROUND_AUTOSCROLL_SPEED : float = 20.0 # Slow downward drift for far background
const STARS_AUTOSCROLL_SPEED : float = 30.0 # Faster downward drift for mid-ground stars

@onready var background_layer : Parallax2D = $BackgroundLayer
@onready var stars_layer : Parallax2D = $StarsLayer
@onready var stage_01 : Node2D = $Stage01
